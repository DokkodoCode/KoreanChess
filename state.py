...
class Multiplayer(MultiplayerPreGameSettings):
    ...
    def __init__(self, window):
        ...
        # Ask if host or client in terminal
        ...
        if choice == 'h':
            ...
            # Wait for client - this will block until a client connects
            ...
            # Test connection
            self.connection.send("Hello from host!")
            response = self.connection.receive()
            print(f"Host received: {response}")
            
        elif choice == 'c':
            print("Starting as client. Connecting to host...")
            host = "127.0.0.1"  # Always use localhost for testing
            port = 12345
            ...
            # Set up players
            self.host = player.Player(is_host=False, board_perspective="Bottom")
            self.guest = player.Player(is_host=True, board_perspective="Top")
            
            # Test connection
            response = self.connection.receive()
            print(f"Client received: {response}")
            self.connection.send("Hello from client!")
